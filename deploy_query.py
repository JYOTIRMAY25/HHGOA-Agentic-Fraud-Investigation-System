#!/usr/bin/env python3
"""Deploy shortestPath query and verify."""
import os
...
# Deploy shortestPath query
query = (
    'CREATE QUERY shortestPath(STRING start_txn_id, STRING end_txn_id) '
    'FOR HHGOA_Fraud_Graph {'
    ...
    'IF (total_distance < 10000) THEN PRINT path;'
    '}'
)
...
try:
    ...
    print(f"installQueries error: {type(e).__name__}: {str(e)[:200]}")

# Check installed queries
print("\nChecking installed queries...")
try:
    r = conn.getInstalledQueries()
    print(f"getInstalledQueries: {str(r)[:500]}")
except Exception as e:
    print(f"getInstalledQueries error: {type(e).__name__}: {str(e)[:200]}")
