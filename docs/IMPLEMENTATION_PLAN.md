...
| **Phase 2** | Live TigerGraph Cluster Ingestion | **BLOCKED** | Credentials are present, but the Savanna workspace is stopped and auto-start is disabled. No schema, data, or queries were changed. (`docs/MCP_LIVE_VALIDATION.md`, `docs/TIGERGRAPH_DEPLOYMENT.md`) |
| **Phase 2** | GSQL Investigation Queries | **COMPLETE** | `scripts/tigergraph/queries/*.gsql`, `tigergraph/gsql/*.gsql` |
...
| **Phase 3** | TigerGraph MCP Integration | **VALIDATED (MOCK); LIVE BLOCKED** | Six tools pass offline fixtures; live validation is blocked because the Savanna workspace is stopped and auto-start is disabled. (`docs/MCP_LIVE_VALIDATION.md`) |
...
- [ ] Connect to active TigerGraph cluster and load 590,742 transactions. **[BLOCKED]** *(Savanna workspace is stopped and auto-start is disabled; credentials are present but no live graph/data verification is possible)*
...
- [x] Install TigerGraph MCP server dependencies. **[COMPLETE]** (pyTigerGraph v2.0.4 available; custom investigation path validated)
- [x] Expose GSQL investigation queries as callable agent tools via MCP. **[COMPLETE]** (six wrappers in `mcp/tools/investigation.py`)
- [x] Validate MCP tool schemas and JSON-RPC parameter passing in mock mode. **[COMPLETE]** (all six tools pass offline fixtures)
- [ ] Validate MCP tool schemas and JSON-RPC parameter passing against live TigerGraph. **[BLOCKED]** *(Savanna workspace is stopped and auto-start is disabled; see `docs/MCP_LIVE_VALIDATION.md`)*
